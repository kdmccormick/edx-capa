    ...
    for problem_file in args.files:
        ...
        try:
            problem = LoncapaProblem(
                problem_file, "fakeid", seed=args.seed, capa_system=system
            )
...
def check_that_blanks_fail(problem):
    ...
    blank_answers = dict(
        (answer_id, u"") for answer_id in problem.get_question_answers()
    )
    ...
    try:
        ...
        log.error(
            "Blank accepted as correct answer in {0} for {1}".format(
                ...
                [
                    answer_id
                    for answer_id, result in sorted(grading_results.items())
                    if result != "incorrect"
                ],
            )
        )
...
def check_that_suggested_answers_work(problem):
    ...
    * "a or d" is what's displayed, but only "a" or "d" is accepted, not the
    ...
    all_answers = dict(
        (answer_id, real_answers.get(answer_id, "")) for answer_id in all_answer_ids
    )
    ...
    if real_answers:
        try:
            ...
            assert all(
                result == "correct" for answer_id, result in real_results.items()
            )
            ...
            log.error(
                "The following generated answers were not accepted for {0}:".format(
                    problem
                )
            )
            for question_id, result in sorted(real_results.items()):
                if result != "correct":
                    log.error(
                        "  {0} = {1}".format(question_id, real_answers[question_id])
                    )
        except Exception as ex:
