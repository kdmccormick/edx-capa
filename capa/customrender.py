    ...
    def get_html(self):
        ...
        try:
            ...
            if self.system.DEBUG:
                ...
                # xss-lint: disable=python-interpolate-html
                msg += (
                    "<p>Failed to construct math expression from <pre>%s</pre></p>"
                    % html.replace("<", "&lt;")
                )  # xss-lint: disable=python-custom-escape
                msg += "</div></html>"
                log.error(msg)
...
class TargetedFeedbackRenderer(object):
    ...
    def get_html(self):
        ...
        html_str = (
            '<section class="targeted-feedback-span"><span>{}</span></section>'.format(
...
class ClarificationRenderer(object):
    ...
    def __init__(self, system, xml):
        ...
        self.inner_html = initial_text + u"".join(
            etree.tostring(element, encoding="unicode") for element in xml
        )
        self.tail = xml.tail
