...
class LoncapaResponse(six.with_metaclass(abc.ABCMeta, object)):
    ...
    def __init__(self, xml, inputfields, context, system, capa_module, minimal_init):
        ...
        for abox in inputfields:
            if abox.tag not in self.allowed_inputfields:
                msg = "%s: cannot have input field %s" % (six.text_type(self), abox.tag)
        ...
        for prop in self.required_attributes:
            if not xml.get(prop):
                msg = (
                    "Error in problem specification: %s missing required attribute %s"
        ...
        if not minimal_init:
            ...
            for entry in self.inputfields:
                ...
                if answer:
                    self.default_answer_map[entry.get("id")] = contextualize_text(
                        answer, self.context
                    )
    ...
    def render_html(self, renderer, response_msg=""):
        ...
        """
        _ = _ = self.capa_system.i18n.gettext
        ...
        if self.xml.get("multiple_inputtypes"):
            ...
            if self.xml.get("multiinput-group-label-id"):
                content.set(
                    "aria-labelledby", self.xml.get("multiinput-group-label-id")
                )

            if self.xml.get("multiinput-group_description_ids"):
                content.set(
                    "aria-describedby", self.xml.get("multiinput-group_description_ids")
                )
        else:
            content = tree
    ...
    def make_hint_div(
        ...
        if not hint_log:
            ...
            if hint_node is None or (
                hint_node.text is None and len(hint_node.getchildren()) == 0
            ):
                return ""
        ...
        self.capa_module.runtime.track_function(
            "edx.problem.hint.feedback_displayed", event_info
        )
        ...
        hints_wrap = HTML("").join(
            [
                HTML(
                    '<div class="{question_hint_text_style}">{hint_content}</div>'
                ).format(
        ...
        if multiline_mode:
            hints_wrap = HTML(
                '<div class="{question_hint_multiline}">{hints_wrap}</div>'
            ).format(
        ...
        if label:
            label_wrap = HTML(
                '<span class="{question_hint_label_style}">{label} </span>'
            ).format(
        ...
        # Ready to go
        return HTML(
            '<div class="{st}"><div class="explanation-title">{text}</div>{lwrp}{hintswrap}</div>'
        ).format(
    ...
    def get_hints(self, student_answers, new_cmap, old_cmap):
        ...
        if hint_function_provided:
            ...
            try:
                ...
                _ = _ = self.capa_system.i18n.gettext
                msg = _("Error {err} in evaluating hint function {hintfn}.").format(
                    err=err, hintfn=hintfn
                )
                sourcenum = getattr(
                    self.xml, "sourceline", _("(Source code line unavailable)")
                )
                msg += "\n" + _("See XML source line {sourcenum}.").format(
                    sourcenum=sourcenum
                )
    ...
    def _render_response_msg_html(self, response_msg):
        ...
        *response_msg* is a string, which may contain XHTML markup
        ...
        # it as an XHTML tree
        try:
            response_msg_div = etree.XML(
                HTML("<div>{}</div>").format(HTML(str(response_msg)))
            )
...
class ChoiceResponse(LoncapaResponse):
    ...
    def grade_via_every_decision_counts(self, **kwargs):
        ...
        good_answers = sum(
            [1 for answer in student_answer if answer in self.correct_choices]
        )
        good_non_answers = sum(
            [1 for blank in student_non_answers if blank in self.incorrect_choices]
        )
        edc_current_grade = good_answers + good_non_answers

        return_grade = round_away_from_zero(
            self.get_max_score() * float(edc_current_grade) / float(edc_max_grade), 2
        )
        ...
        if edc_current_grade == edc_max_grade:
            ...
            return CorrectMap(
                self.answer_id, correctness="partially-correct", npoints=return_grade
            )
        else:
    ...
    def grade_via_halves(self, **kwargs):
        ...
        incorrect_answers = sum(
            [1 for answer in student_answer if answer in self.incorrect_choices]
        )
        missed_answers = sum(
            [1 for blank in student_non_answers if blank in self.correct_choices]
        )
        ...
        if halves_error_count == 0:
            ...
            return CorrectMap(
                self.answer_id, correctness="correct", npoints=return_grade
            )
    ...
    def get_score(self, student_answers):
        ...
        if len(self.credit_type) > 1:
            raise LoncapaProblemError(
                "Only one type of partial credit is allowed for Checkbox problems."
            )

        # Make sure we're using an approved style.
        if self.credit_type[0] not in graders:
            raise LoncapaProblemError(
                "partial_credit attribute should be one of: " + ",".join(graders)
            )
        ...
        for choice in choices:
            ...
            for hint_node in hint_nodes:
                if hint_node.get("selected", "").lower() == selector:
                    ...
                    if (
                        hint_node.get("label") is not None
                    ):  # tricky: label '' vs None is significant
                    ...
                    if text:
                        hint_log.append(
                            {
                                "text": text,
                                "trigger": [{"choice": name, "selected": selected}],
                            }
                        )
    ...
    def get_compound_hints(self, new_cmap, student_answers):
        ...
        if self.answer_id in student_answers:
            ...
            for compound_hint in self.xml.xpath(
                "//checkboxgroup[@id=$id]/compoundhint", id=self.answer_id
            ):
                ...
                if selector_set == student_set:
                    ...
                    choices = self.xml.xpath(
                        "//checkboxgroup[@id=$id]/choice", id=self.answer_id
                    )
                    ...
                    hint_log = [
                        {
                            "text": hint_text,
                            "trigger": [
                                {"choice": name, "selected": True} for name in names
                            ],
                        }
                    ]
                    new_cmap[self.answer_id]["msg"] += self.make_hint_div(
...
class MultipleChoiceResponse(LoncapaResponse):
    ...
    def setup_response(self):
        ...
        if self.has_partial_credit:
            self.partial_choices = [
                contextualize_text(choice.get("name"), self.context)
                for choice in cxml
                if contextualize_text(choice.get("correct"), self.context).lower()
                == "partial"
            ]
            self.partial_values = [
                float(
                    choice.get("point_value", default="0.5")
                )  # Default partial credit: 50%
                for choice in cxml
                if contextualize_text(choice.get("correct"), self.context).lower()
                == "partial"
            ]
    ...
    def get_extended_hints(self, student_answer_dict, new_cmap):
        ...
        if self.answer_id in student_answer_dict:
            ...
            choice = self.xml.find(
                './choicegroup[@id="{0}"]/choice[@name="{1}"]'.format(
                    self.answer_id, student_answer
                )
            )
    ...
    def grade_via_points(self, **kwargs):
        ...
        if (
            ...
            return CorrectMap(
                self.answer_id, correctness="partially-correct", npoints=credit_amount
            )
        else:
            return CorrectMap(self.answer_id, "incorrect")
    ...
    def get_score(self, student_answers):
        ...
        if len(self.credit_type) > 1:
            raise LoncapaProblemError(
                "Only one type of partial credit is allowed for Multiple Choice problems."
            )
    ...
    def do_answer_pool(self, tree, problem):
        ...
        if choicegroups:
            ...
            (solution_id, subset_choices) = self.sample_from_answer_pool(
                choices_list, rng, num_choices
            )
...
class OptionResponse(LoncapaResponse):
    ...
    def get_score(self, student_answers):
        ...
        for aid in amap:
            ...
            answer_variable = self.get_student_answer_variable_name(
                student_answers, aid
            )
            if answer_variable:
    ...
    def get_answers(self):
        amap = dict(
            [
                (af.get("id"), contextualize_text(af.get("correct"), self.context))
    ...
    def get_extended_hints(self, student_answers, new_cmap):
        ...
        if answer_id in student_answers:
            ...
            options = [
                option for option in options if option.text.strip() == student_answer
            ]
            if options:
                option = options[0]
...
class NumericalResponse(LoncapaResponse):
    ...
    def setup_response(self):
        ...
        if answer.startswith(("[", "(")) and answer.endswith(
            ...
            try:
                self.answer_range = [
                    contextualize_text(x, context) for x in answer[1:-1].split(",")
                ]
                self.correct_answer = (
                    answer[0]
                    + self.answer_range[0]
                ...
                log.debug(
                    "Content error--answer '%s' is not a valid range tolerance answer",
                    answer,
                )
                _ = _ = self.capa_system.i18n.gettext
    ...
    def get_score(self, student_answers):
        ...
        if self.has_partial_credit:
            ...
            for style in self.credit_type:
                if style not in graders:
                    raise LoncapaProblemError(
                        "partial_credit attribute should be one of: "
                        + ",".join(graders)
                    )
        ...
        try:
            ...
            raise StudentInputError(err.args[0])
            ...
            if "factorial" in text_type(val_err):
                ...
                raise StudentInputError(
                    _(
                        ...
                        "'{student_answer}'"
                    ).format(student_answer=html.escape(student_answer))
                )
            else:
                raise general_exception
        except ParseException:
            raise StudentInputError(
                _(u"Invalid math syntax: '{student_answer}'").format(
                    student_answer=html.escape(student_answer)
                )
            )
        except Exception:
        ...
        if has_partial_range:
            partial_range = float(
                has_partial_range[0].get("partial_range", default="2")
            )
        else:
            partial_range = 2
        ...
        if self.range_tolerance:
            if isinstance(student_float, complex):
                raise StudentInputError(
                    _(u"You may not use complex numbers in range tolerance problems")
                )
            boundaries = []
            for inclusion, answer in zip(self.inclusion, self.answer_range):
                ...
                if boundaries[0] < student_float < boundaries[1]:
                    ...
                    if self.has_partial_credit is False:
                        ...
                        extended_boundaries.append(
                            boundaries[0] - partial_range * boundary_range
                        )
                        extended_boundaries.append(
                            boundaries[1] + partial_range * boundary_range
                        )
                        if (
                            extended_boundaries[0]
                            < student_float
            ...
            if str(self.tolerance).endswith("%"):
                expanded_tolerance = (
                    str(partial_range * float(str(self.tolerance)[:-1])) + "%"
                )
            else:
            ...
            if compare_with_tolerance(student_float, correct_float, self.tolerance):
                ...
                for value in partial_answers:
                    if compare_with_tolerance(student_float, value, self.tolerance):
                        ...
                        if compare_with_tolerance(
                            student_float, correct_float, expanded_tolerance
                        ):
                            is_correct = "partially-correct"
                            break
                        elif compare_with_tolerance(
                            student_float, value, expanded_tolerance
                        ):
                            is_correct = "partially-correct"
    ...
    def compare_answer(self, ans1, ans2):
        ...
        return compare_with_tolerance(
            evaluator({}, {}, ans1), evaluator({}, {}, ans2), self.tolerance
        )
    ...
    def get_answers(self):
        ...
        return {
            self.answer_id: separator.join(
                [self.correct_answer] + self.additional_answers
            )
        }
    ...
    def set_cmap_msg(self, student_answers, new_cmap, hint_type, hint_index):
        ...
        hint_nodes = self.xml.xpath(
            "//numericalresponse[@id=$id]/" + hint_type, id=self.id
        )
        if hint_nodes:
    ...
    def get_extended_hints(self, student_answers, new_cmap):
        ...
        if self.answer_id in student_answers:
            if (
                new_cmap.cmap[self.answer_id]["correctness"] == "correct"
            ):  # if the grader liked the student's answer
...
class StringResponse(LoncapaResponse):
    ...
    def setup_response_backward(self):
        self.correct_answer = [
            contextualize_text(answer, self.context).strip()
        ...
        if self.answer_id in student_answers:
            ...
            if responses:
                ...
                for node in response.findall("./additional_answer"):
                    if self.match_hint_node(
                        node, student_answer, self.regexp, self.case_insensitive
                    ):
                ...
                for hint_node in response.findall("./stringequalhint"):
                    if self.match_hint_node(
                        hint_node, student_answer, False, self.case_insensitive
                    ):
                        new_cmap[self.answer_id]["msg"] += self.make_hint_div(
                ...
                for hint_node in response.findall("./regexphint"):
                    if self.match_hint_node(
                        hint_node, student_answer, True, self.case_insensitive
                    ):
                        new_cmap[self.answer_id]["msg"] += self.make_hint_div(
                            hint_node, False, [student_answer], self.tags[0]
                        )
...
class CustomResponse(LoncapaResponse):
    ...
    def setup_response(self):
        ...
        # that
        self.expect = contextualize_text(
            xml.get("expect") or xml.get("answer"), self.context
        )
        ...
        if not self.code:
            if answer is None:
                ...
                if answer_src is not None:
                    ...
                    self.code = self.capa_system.filesystem.open(
                        "src/" + answer_src
                    ).read()
                else:
    ...
    def get_score(self, student_answers):
        ...
        if len(idset) == 1 and not submission[0]:
            ...
            msg = (
                HTML(u'<span class="inline-error">{0}</span>').format(
                    _(u"No answer entered!")
                )
        ...
        if isinstance(self.code, six.string_types):
            ...
            if isinstance(ret, dict):
                ...
                if "ok" in ret:
                    ...
                    ok_val = (
                        str(ret["ok"]).lower().strip() if bool(ret["ok"]) else "false"
                    )
                ...
                # status, scaled grades, and messages for individual inputs
                    ...
                    for input_dict in input_list:
                        if (
                            str(input_dict["ok"]).lower().strip() == "false"
                            or not input_dict["ok"]
                        ):
                        ...
                        msg = (
                            self.clean_message_html(input_dict["msg"])
                            if "msg" in input_dict
                            else None
                        )
                        messages.append(msg)
...
class SymbolicResponse(CustomResponse):
    ...
    def execute_check_function(self, idset, submission):
        ...
        try:
            ...
            msg = _(
                u"An error occurred with SymbolicResponse. The error was: {error_msg}"
            ).format(
                error_msg=err,
            )
            raise Exception(msg)
...
class CodeResponse(LoncapaResponse):
    ...
    def _parse_coderesponse_xml(self, codeparam):
        ...
        self.answer = find_with_default(
            codeparam, "answer_display", _(u"No answer provided.")
        )
    ...
    def get_score(self, student_answers):
        ...
        if error:
            ...
            cmap.set(
                self.answer_id, queuestate=queuestate, correctness="incomplete", msg=msg
            )
    ...
    def get_answers(self):
        anshtml = HTML(
            '<span class="code-answer"><pre><code>{}</code></pre></span>'
        ).format(self.answer)
        ...
        try:
            ...
            try:
                parsed = html5lib.parseFragment(
                    msg, treebuilder="lxml", namespaceHTMLElements=False
                )
            except ValueError:
...
class ExternalResponse(LoncapaResponse):
    ...
    def do_external_request(self, cmd, extra_payload):
        ...
        try:
            ...
            msg = "Error {0} - cannot connect to external server url={1}".format(
                err, self.url
            )
            log.error(msg)
            raise Exception(msg)
        ...
        try:
            ...
            msg = "Error {0} - cannot parse response from external server req.text={1}".format(
                err, req.text
            )
            log.error(msg)
            raise Exception(msg)
    ...
    def get_score(self, student_answers):
        ...
        try:
            ...
            if self.capa_system.DEBUG:
                cmap.set_dict(
                    dict(list(zip(sorted(self.answer_ids), ["incorrect"] * len(idset))))
                )
                cmap.set_property(
                    self.answer_ids[0],
                    "msg",
        ...
        for key in idset:
            idx = idset.index(key)
            msg = (
                rxml.find("message").text.replace("&nbsp;", "&#160;")
                if idx == 0
                else None
            )
    ...
    def get_answers(self):
        ...
        if not len(exans) == len(self.answer_ids):
            log.error(
                "Expected %s answers from external server, only got %s!",
                len(self.answer_ids),
                len(exans),
            )
...
class FormulaResponse(LoncapaResponse):
    ...
    def setup_response(self):
        ...
        # Find the tolerance
        tolerance_xml = xml.xpath(
            '//*[@id=$id]//responseparam[@type="tolerance"]/@default', id=xml.get("id")
        )
        if tolerance_xml:  # If it isn't an empty list...
            self.tolerance = contextualize_text(tolerance_xml[0].strip(), context)
        ...
        sranges = list(
            zip(
                *[
                    ...
                    for x in samples.split("@")[1].split("#")[0].split(":")
                ]
            )
        )
    ...
    def strip_dict(self, inp_d):
        ...
        inp_d = dict(
            [
                ...
                if isinstance(k, str)
                and k.isalnum()
                and isinstance(inp_d[k], numbers.Number)
            ]
        )
        return inp_d
...
class ImageResponse(LoncapaResponse):
            ...
            if rectangles[aid]:  # Rectangles part - for backward compatibility
                ...
                for solution_rectangle in solution_rectangles:
                    ...
                    if not sr_coords:
                        ...
                        msg = _(
                            "Error in problem specification! Cannot parse rectangle in {sr_coords}"
                        ).format(
                            sr_coords=etree.tostring(
                                self.ielements[aid], pretty_print=True
                            )
                        )
                        raise Exception(
            ...
            if correct_map[aid]["correctness"] != "correct" and regions[aid]:
                ...
                if parsed_region:
                    ...
                    for region in parsed_region:
                        ...
                        if polygon.type == "Polygon" and polygon.contains(
                            Point(ans_x, ans_y)
                        ):
                            correct_map.set(aid, "correct")
                            break
    ...
    def get_mapped_answers(self):
        ...
        """
        answers = (
            dict([(ie.get("id"), ie.get("rectangle")) for ie in self.ielements]),
    ...
    def get_answers(self):
        ...
        for ielt in self.ielements:
            ie_id = ielt.get("id")
            answers[ie_id] = {
                "rectangle": ielt.get("rectangle"),
                "regions": ielt.get("regions"),
            }
...
class ChoiceTextResponse(LoncapaResponse):
    ...
    def setup_response(self):
        ...
        correct_xml = self.xml.xpath(
            '//*[@id=$id]//choice[@correct="true"]', id=self.xml.get("id")
        )
        ...
        for node in correct_xml:
            ...
            for child in node:
                ...
                if not answer:
                    ...
                    raise LoncapaProblemError(
                        _("Answer not provided for {input_type}").format(
                            input_type="numtolerance_input"
                        )
                    )
                ...
                tolerance = contextualize_text(
                    child.get("tolerance", default_tolerance), context
                )
    ...
    def _split_answers_dict(self, a_dict):
        ...
        selected_numtolerance_inputs = [
            key
            for key in a_dict
            if key.partition("_numtolerance_input_")[0] + "bc" in selected_choices
        ]
        ...
        for answer_name, answer_value in six.iteritems(numtolerance_inputs):
            ...
            try:
                partial_correct = compare_with_tolerance(
                    evaluator({}, {}, answer_value), correct_ans, tolerance
                )
            except:
