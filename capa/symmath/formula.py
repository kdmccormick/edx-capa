...
def to_latex(expr):
    ...
    expr_s = re.sub(r"script([a-zA-Z0-9]+)", r"\\mathcal{\\1}", expr_s)

    # return '<math>%s{}{}</math>' % (xs[1:-1])
    if expr_s[0] == "$":
        return "[mathjax]%s[/mathjax]<br>" % (
            expr_s[1:-1]
        )  # for sympy v6  # xss-lint: disable=python-interpolate-html
    return "[mathjax]%s[/mathjax]<br>" % (
        expr_s
    )  # for sympy v7  # xss-lint: disable=python-interpolate-html
...
def my_sympify(
    expr, normphase=False, matrix=False, abcsym=False, do_qubit=False, symtab=None
):
    """
...
class formula(object):
        ...
        def fix_msubsup(parent):
            ...
            for child in parent:
                ...
                if gettag(child) == "msubsup" and len(child) == 3:
                    ...
                    newbase.text = "%s_%s" % (
                        flatten_pmathml(child[0]),
                        flatten_pmathml(child[1]),
                    )
                    newexp = child[2]
    ...
    def make_sympy(self, xml=None):
        ...
        if tag == "apply":  # apply operator
            ...
            if opstr in opdict:
                ...
                try:
                    ...
                    raise Exception(
                        "[formula] error=%s failed to apply %s to args=%s"
                        % (err, opstr, args)
                    )
                return res
            else:
        ...
        # atoms are below
            ...
            if len(xml) > 0 and (
                gettag(xml[0]) == "msub" or gettag(xml[0]) == "msup"
