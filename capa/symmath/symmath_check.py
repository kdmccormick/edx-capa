...
def symmath_check_simple(expect, ans, adict={}, symtab=None, extra_options=None):
    ...
    try:
        ...
        return {
            "ok": False,
            "msg": "Error %s<br/>Failed in evaluating check(%s,%s)"
            % (err, expect, ans),
        }
    return ret
...
def check(
    ...
    try:
        xgiven = my_sympify(
            given, normphase, matrix, do_qubit=do_qubit, abcsym=abcsym, symtab=symtab
        )
    except Exception as err:
        return {
            "ok": False,
            "msg": 'Error %s<br/> in evaluating your expression "%s"' % (err, given),
        }

    try:
        xexpect = my_sympify(
            expect, normphase, matrix, do_qubit=do_qubit, abcsym=abcsym, symtab=symtab
        )
    except Exception as err:
        return {
            "ok": False,
            "msg": 'Error %s<br/> in evaluating OUR expression "%s"' % (err, expect),
        }
    ...
    if "autonorm" in flags:  # normalize trace of matrices
        try:
            ...
            return {
                "ok": False,
                "msg": "Error %s<br/> in normalizing trace of your expression %s"
                % (err, to_latex(xgiven)),
            }
        try:
            ...
            return {
                "ok": False,
                "msg": "Error %s<br/> in normalizing trace of OUR expression %s"
                % (err, to_latex(xexpect)),
            }
...
def symmath_check(expect, ans, dynamath=None, options=None, debug=None, xml=None):
    ...
    if xml is not None:
        DEBUG = xml.get(
            "debug", False
        )  # override debug flag using attribute in symbolicmath xml
        if DEBUG in ["0", "False"]:
            DEBUG = False
    ...
    if (
        hasattr(fexpect, "is_number")
        and fexpect.is_number
        and hasattr(fans, "is_number")
        and fans.is_number
    ):
    ...
    if do_numerical:  # numerical answer expected - force numerical comparison
        if is_within_tolerance(fexpect, fans, threshold):
            ...
            msg += (
                "<p>You entered: %s (note that a numerical answer is expected)</p>"
                % to_latex(fans)
            )
            return {"ok": False, "msg": msg}
    ...
    if not mmlans:
        return {
            "ok": False,
            "msg": "[symmath_check] failed to get MathML for input; dynamath=%s"
            % dynamath,
        }
    ...
    try:
        ...
        msg += "<p>Error in evaluating your expression '%s' as a valid equation</p>" % (
            ans
        )
    ...
    if DEBUG:
        ...
        msg += "<p>Expecting: %s</p>" % repr(fexpect).replace("**", "^").replace(
            "hat(I)", "hat(i)"
        )
