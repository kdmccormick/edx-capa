    ...
    def test_trailing_text_rendering(self):
        ...
        for xml_text, expected_text in trailing_text:
            ...
            expected = {
                ...
                "preprocessor": None,
                "response_data": RESPONSE_DATA,
                "describedby_html": TRAILING_TEXT_DESCRIBEDBY.format(
                    trailing_text_id=prob_id, status_id=prob_id
                ),
            }
            self.assertEqual(context, expected)
...
class MatlabTest(unittest.TestCase):
    ...
    def test_plot_data(self):
        ...
        test_capa_system().xqueue["interface"].send_to_queue.assert_called_with(
            header=ANY, body=ANY
        )
    ...
    def test_plot_data_failure(self):
        ...
        test_capa_system().xqueue["interface"].send_to_queue.return_value = (
            1,
            error_message,
        )
        response = self.the_input.handle_ajax("plot", data)
    ...
    def test_matlab_response_timeout_exceeded(self, time):
        ...
        self.assertEqual(
            the_input.msg,
            "No response from Xqueue within {} seconds. Aborted.".format(
                XQUEUE_TIMEOUT
            ),
        )
    ...
    def test_get_html(self):
        ...
        six.assertCountEqual(
            self,
            element_keys[audio_index],
            ["autobuffer", "controls", "autoplay", "src"],
        )
        self.assertEqual(elements[audio_index].get("src"), "data:audio/wav;base64=")
        self.assertEqual(
            elements[audio_index].text, "Audio is not supported on this browser."
        )
        href_index = element_keys.index(["href"])
        self.assertEqual(
            elements[href_index].get("href"),
            "https://endpoint.mss-mathworks.com/media/filename.wav",
        )
    ...
    def test_matlab_sanitize_msg(self):
        ...
        expected = "&lt;script&gt;Test message&lt;/script&gt;"
        self.assertEqual(
            self.the_input._get_render_context()["msg"], expected
        )  # pylint: disable=protected-access
...
class ChemicalEquationTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.the_input = lookup_tag("chemicalequationinput")(
            test_capa_system(), element, state
        )
    ...
    def test_ajax_parse_err(self):
        ...
        with patch("capa.inputtypes.chemcalc.render_to_html") as mock_render:
            ...
            response = self.the_input.handle_ajax(
                "preview_chemcalc", {"formula": "H2O + invalid chemistry"}
            )
    ...
    def test_ajax_other_err(self, mock_log):
        ...
        with patch("capa.inputtypes.chemcalc.render_to_html") as mock_render:
            ...
            response = self.the_input.handle_ajax(
                "preview_chemcalc", {"formula": "H2O + superterrible chemistry"}
            )
...
class FormulaEquationTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.size = "42"
        xml_str = """<formulaequationinput id="prob_1_2" size="{size}"/>""".format(
            size=self.size
        )
        ...
        self.the_input = lookup_tag("formulaequationinput")(
            test_capa_system(), element, state
        )
...
class TestChoiceText(unittest.TestCase):
  ...
  <{tag}>
      ...
      <choice correct="true" name="choiceinput_1">Is a number<decoy_input name="choiceinput_1_textinput_0"/><text>!</text></choice>
        ...
        first_input = self.build_choice_element(
            "numtolerance_input", "choiceinput_0_textinput_0", "false", ""
        )
        second_input = self.build_choice_element(
            "decoy_input", "choiceinput_1_textinput_0", "", ""
        )
        ...
        choices = [
            ...
            (
                "choiceinput_1",
                [second_choice_content, second_input, second_choice_text],
            ),
        ]
        expected = {
            "STATIC_URL": "/dummy-static/",
