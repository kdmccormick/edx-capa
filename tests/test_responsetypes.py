...
class ResponseTest(unittest.TestCase):
    ...
    def assert_grade(self, problem, submission, expected_correctness, msg=None):
        ...
        if msg is None:
            ...
            self.assertEqual(
                correct_map.get_correctness("1_2_1"), expected_correctness, msg
            )
    ...
    def assert_multiple_partial(
        self, problem, correct_answers, incorrect_answers, partial_answers
    ):
        """
...
class MultiChoiceResponseTest(ResponseTest):  # pylint: disable=missing-class-docstring
    ...
    def test_partial_multiple_choice_grade(self):
        problem = self.build_problem(
            choices=[False, True, "partial"], credit_type="points"
        )
    ...
    def test_named_multiple_choice_grade(self):
        problem = self.build_problem(
            choices=[False, True, False], choice_names=["foil_1", "foil_2", "foil_3"]
        )
    ...
    def test_multiple_choice_valid_grading_schemes(self):
        ...
        problem = self.build_problem(
            choices=[False, True, "partial"], credit_type="points,points"
        )
        ...
        problem = self.build_problem(
            choices=[False, True, "partial"], credit_type="bongo"
        )
        with self.assertRaises(LoncapaProblemError):
            input_dict = {"1_2_1": "choice_1"}
            problem.grade_answers(input_dict)
    ...
    def test_contextualized_choices(self):
        ...
        self.assert_grade(
            problem, "choice_2 + 9 is even ... (should be False)", "incorrect"
        )
        self.assert_grade(
            problem, "choice_2 + 9 is odd ... (should be True)", "correct"
        )
        self.assert_grade(
            problem,
            "choice_infinity may be both ... (should be partial)",
            "partially-correct",
        )
...
class TrueFalseResponseTest(ResponseTest):  # pylint: disable=missing-class-docstring
    ...
    def test_named_true_false_grade(self):
        problem = self.build_problem(
            choices=[False, True, True], choice_names=["foil_1", "foil_2", "foil_3"]
        )
        ...
        self.assert_grade(problem, "choice_foil_3", "incorrect")
        self.assert_grade(
            problem, ["choice_foil_1", "choice_foil_2", "choice_foil_3"], "incorrect"
        )
...
class OptionResponseTest(ResponseTest):  # pylint: disable=missing-class-docstring
    ...
    def test_grade(self):
        problem = self.build_problem(
            options=["first", "second", "third"], correct_option="second"
        )
    ...
    def test_quote_option(self):
        ...
        problem = self.build_problem(
            options=["hasnot", "hasn't", "has'nt"], correct_option="hasn't"
        )
    ...
    def test_variable_options(self):
        ...
        problem = self.build_problem(
            options=["$a", "$b", "$c"], correct_option="$a", script=script
        )
...
class FormulaResponseTest(ResponseTest):
    ...
    def test_grade(self):
        ...
        problem = self.build_problem(
            sample_dict=sample_dict, num_samples=10, tolerance=0.01, answer="x+2*y"
        )
    ...
    def test_hint(self):
        ...
        # or leaves out x
        hints = [
            ("x + 3*y", "y_coefficient", "Check the coefficient of y"),
        ...
        correct_map = problem.grade_answers(input_dict)
        self.assertEqual(correct_map.get_hint("1_2_1"), "Check the coefficient of y")

        # Expect to receive a hint if we leave out x
        input_dict = {"1_2_1": "2*y"}
        correct_map = problem.grade_answers(input_dict)
        self.assertEqual(correct_map.get_hint("1_2_1"), "Try including the variable x")
    ...
    def test_grade_infinity(self):
        ...
        # Test problem
        problem = self.build_problem(
            sample_dict=sample_dict, num_samples=10, tolerance="1%", answer="x"
        )
    ...
    def test_validate_answer(self):
        ...
        self.assertFalse(
            list(problem.responders.values())[0].validate_answer("3*y+2*x")
        )
...
class StringResponseTest(ResponseTest):  # pylint: disable=missing-class-docstring
    ...
    def test_regexp(self):
        ...
        problem = self.build_problem(
            answer="^Sec(ond)?$", case_sensitive=False, regexp=True
        )
        self.assert_grade(problem, "Second", "correct")
        ...
        problem = self.build_problem(
            answer=r"\w*\.?.*Luther King\s*.*", case_sensitive=True, regexp=True
        )
        ...
        problem = self.build_problem(
            answer=r"^(-\|){2,5}$", case_sensitive=False, regexp=True
        )
        ...
        # test unicode
        problem = self.build_problem(
            answer=u"æ", case_sensitive=False, regexp=True, additional_answers=[u"ö"]
        )
    ...
    def test_backslash_and_unicode_regexps(self):
        ...
        """
        problem = self.build_problem(
            answer=u"5\\\\æ", case_sensitive=False, regexp=True
        )
    ...
    def test_backslash(self):
        problem = self.build_problem(
            answer=u"a\\\\c1", case_sensitive=False, regexp=True
        )
        self.assert_grade(problem, u"a\\c1", "correct")

    def test_special_chars(self):
        problem = self.build_problem(
            answer=u"a \\s1", case_sensitive=False, regexp=True
        )
    ...
    def test_case_insensitive(self):
        ...
        answers = ["Second", "Third", "Fourth"]
        problem = self.build_problem(
            answer="sample_answer", case_sensitive=False, additional_answers=answers
        )
    ...
    def test_compatible_non_attribute_additional_answer_xml(self):
        problem = self.build_problem(
            answer="Donut", non_attribute_answers=["Sprinkles"]
        )
    ...
    def test_exception(self):
        problem = self.build_problem(
            answer="a2", case_sensitive=False, regexp=True, additional_answers=["?\\d?"]
        )
    ...
    def test_hints(self):
        ...
        correct_map = problem.grade_answers(input_dict)
        self.assertEqual(
            correct_map.get_hint("1_2_1"), "The state capital of Wisconsin is Madison"
        )
        ...
        correct_map = problem.grade_answers(input_dict)
        self.assertEqual(
            correct_map.get_hint("1_2_1"), "The state capital of Minnesota is St. Paul"
        )
    ...
    def test_hints_regexp_and_answer_regexp(self):
        ...
        for answer in different_student_answers:
            ...
            correct_map = problem.grade_answers(input_dict)
            self.assertEqual(
                correct_map.get_hint("1_2_1"), "First letter of correct answer is M."
            )
...
class CodeResponseTest(ResponseTest):  # pylint: disable=missing-class-docstring
    ...
    def test_update_score(self):
        ...
        for correctness in ["correct", "incorrect"]:
            ...
            self.assertEqual(
                self.problem.correct_map.get_dict(), old_cmap.get_dict()
            )  # Deep comparison

            for answer_id in answer_ids:
                self.assertTrue(
                    self.problem.correct_map.is_queued(answer_id)
        ...
        for correctness in ["correct", "incorrect"]:
            for i, answer_id in enumerate(answer_ids):
                ...
                for j, test_id in enumerate(answer_ids):
                    if j == i:
                        self.assertFalse(
                            self.problem.correct_map.is_queued(test_id)
    ...
    def test_convert_files_to_filenames(self):
        ...
        problem_file = os.path.join(
            os.path.dirname(__file__), "test_files/filename_convert_test.txt"
        )
        with open(problem_file) as fp:
            ...
            self.assertEqual(
                answers_converted["1_3_1"], ["answer1", "answer2", "answer3"]
            )
    ...
    def test_parse_score_msg_of_responder(self):
        ...
        for grader_msg in valid_grader_msgs:
            ...
            for i, answer_id in enumerate(answer_ids):
                self.problem.correct_map = CorrectMap()
                self.problem.correct_map.update(old_cmap)
                output = self.problem.update_score(
                    xserver_msgs["correct"], queuekey=1000 + i
                )
...
class ChoiceResponseTest(ResponseTest):  # pylint: disable=missing-class-docstring
    ...
    def test_checkbox_group_grade(self):
        problem = self.build_problem(
            choice_type="checkbox", choices=[False, True, True]
        )
    ...
    def test_checkbox_group_partial_credit_grade(self):
        ...
        self.assert_grade(problem, "choice_2", "partially-correct")
        self.assert_grade(
            problem, ["choice_1", "choice_2", "choice_3"], "partially-correct"
        )
        self.assert_grade(
            problem, ["choice_0", "choice_1", "choice_2", "choice_3"], "incorrect"
        )
    ...
    def test_contextualized_choices(self):
        ...
        problem = self.build_problem(
            script=script, choice_type="checkbox", choices=choices
        )
...
class NumericalResponseTest(ResponseTest):  # pylint: disable=missing-class-docstring
    ...
    def test_grade_range_tolerance_partial_credit(self):
        problem_setup = [
            ...
            ["[0, 1.6e-5]", ["1.6*10^-5"], ["2"], ["1.9e-5", "-1e-6"]],
            ["(1.6e-5, 10]", ["2"], ["-20", "30"], ["-1", "12"]],
        ]
        for (
            given_answer,
            correct_responses,
            ...
            self.assert_multiple_partial(
                problem, correct_responses, incorrect_responses, partial_responses
            )
    ...
    def test_grade_partial(self):
        ...
        problem = self.build_problem(
            answer=4, credit_type="list", partial_answers="2,8,-4"
        )
        ...
        problem = self.build_problem(answer=4, tolerance=0.2, credit_type="close")
        ...
        problem = self.build_problem(
            answer=4, tolerance=0.2, partial_range=3, credit_type="close"
        )
    ...
    def test_floats(self):
        ...
        problem_setup = [
            ...
            [2e-15, ["2*10^-15"], [""]],
            [3141592653589793238.0, ["3141592653589793115."], [""]],
    ...
    def test_err_handling(self):
        ...
        errors = [  # (exception raised, message to student)
            (
                calc.UndefinedVariable(
                    "Invalid Input: x not permitted in answer as a variable"
                ),
                r"Invalid Input: x not permitted in answer as a variable",
            ),
            (
                ValueError("factorial() mess-up"),
...
class CustomResponseTest(ResponseTest):  # pylint: disable=missing-class-docstring
    ...
    def test_inline_randomization(self):
        ...
        inline_script = "messages[0] = {code}".format(
            code=self._get_random_number_code()
        )
        problem = self.build_problem(answer=inline_script)
    ...
    def test_script_context(self):
        ...
        problems = (
            ...
            self.build_problem(
                script=script, cfn="check_func", answer_attr="$expected_ans"
            ),
        )
    ...
    def test_function_code_multiple_input_no_msg(self):
        ...
        problem = self.build_problem(
            script=script, cfn="check_func", expect="42", num_inputs=2
        )
    ...
    def test_function_code_with_attempt_number(self):
        ...
        problem = self.build_problem(
            script=script, cfn="gradeit", expect="42", cfn_extra_args="attempt"
        )
    ...
    def test_setup_randomization(self):
        ...
        self.assertEqual(
            problem.context["num"], self._get_random_number_result(problem.seed)
        )
    ...
    def test_module_imports_inline(self):
        ...
        for module_name in [
            ...
            try:
                ...
                self.fail(
                    "Could not use name '{0}s' in custom response".format(module_name)
                )
    ...
    def test_function_code_multiple_inputs_order(self):
        ...
        self.assertListEqual(
            list(problem.responders.values())[0].context["idset"], correct_order
        )
...
class ChoiceTextResponseTest(ResponseTest):
    ...
    TEST_PROBLEM_ARGS = {
        ...
        "2_choices_0_inputs": {"choices": [("false", {}), ("true", {})], "script": ""},
        "2_choices_1_input_1": {
            "choices": [("false", {}), ("true", {"answer": "123", "tolerance": "0"})],
            "script": "",
        },
        "2_choices_1_input_2": {
    ...
    def test_unchecked_input_not_validated(self):
        ...
        two_choice_two_input = self._make_problem(
            [("true", {"answer": "123", "tolerance": "1"}), ("false", {})],
            "checkboxtextgroup",
        )
    ...
    def test_staff_answer_error(self):
        ...
        with self.assertRaisesRegex(
            StudentInputError, "The Staff answer could not be interpreted as a number."
        ):
            self.assert_grade(
                broken_problem,
    ...
    def test_radio_grades(self):
        ...
        for name, inputs in six.iteritems(self.TEST_INPUTS):
            ...
            test_problem = self._make_problem(
                test_choices, "radiotextgroup", test_script
            )
    ...
    def test_checkbox_grades(self):
        ...
        scenarios = {
            ...
            "2_choices_2_inputs_correct": ("checkbox_2_choices_2_inputs", "correct"),
        ...
        inputs = {
            ...
            "2_choices_2_inputs_missing_choice": [(True, ["123"]), (False, ["456"])],
            "2_choices_2_inputs_wrong_input": [(True, ["123"]), (True, ["654"])],
        }
